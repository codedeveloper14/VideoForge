...
def test_script_con_idea_valida_usa_template(client, monkeypatch):
    ...
    resp = client.post("/api/idea2video/script",
                        json={"idea": "el impacto de la IA", "dur": 30, "style": "tutorial"})
    assert resp.status_code == 200
    body = resp.get_json()
...
def test_ap_imagen_bloquea_traversal(client, monkeypatch):
    _login(client, monkeypatch)
    resp = client.get("/api/idea2video/ap_imagen",
...
def test_ap_imagen_sin_archivo_da_404(client, monkeypatch):
    _login(client, monkeypatch)
    resp = client.get("/api/idea2video/ap_imagen",
                       query_string={"project": "proyecto_que_no_existe", "file": "a.png"})
    assert resp.status_code == 404
