...
def _login_as(client, monkeypatch, username="usuario_test", role="user"):
    ...
    resp = client.post(
        "/api/login",
        json={"username": username, "password": "clave-correcta-123"},
...
def test_help_submit_funciona_sin_auth(client, monkeypatch):
    calls = []
    monkeypatch.setattr(docs_repository, "insert_help_report", lambda *a: calls.append(a))
...
def test_admin_docs_rechaza_usuario_no_admin(client, monkeypatch):
    ...
    monkeypatch.setattr(user_repository, "get_user_full", lambda u: {"role": "user", "username": u})
...
def test_admin_docs_permite_admin(client, monkeypatch):
    ...
    monkeypatch.setattr(user_repository, "get_user_full", lambda u: {"role": "admin", "username": u})
