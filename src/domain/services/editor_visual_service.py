...
def _read_valid_timestamps(project_name: str) -> list[dict]:
    ...
    logger.info(
        "editor_visual: timestamps cacheados rechazados (avg=%.2fs gaps=%d/%d) - se eliminan",
        avg_dur,
        gaps,
        len(raw) - 1,
    )
    ...
    image_paths = (
        sorted(
            (
                ...
                if f.is_file() and f.suffix.lower().lstrip(".") in _EDITOR_IMG_EXTS
            ),
            key=scene_sort_key,
        )
        if img_dir.exists()
...
def transcribe_project(project_name: str) -> dict:
    ...
    if cached:
        segs_out = [
            {
                "start": float(t.get("inicio", 0)),
                "end": float(t.get("fin", 0)),
                "text": t.get("texto", ""),
                "seg_idx": t.get("escena", 1) - 1,
            }
            for t in cached
        ]
    ...
    ts_records = [
        {
            "escena": i + 1,
            "seg_idx": i,
            "texto": guion_lines[i] if i < len(guion_lines) else "",
            "inicio": t["inicio"],
            "fin": t["fin"],
            "duracion": t["duracion"],
    ...
    segs_out = [
        {"start": t["inicio"], "end": t["fin"], "text": t["texto"], "seg_idx": i}
