...
_REF_TIPOS = {"broll", "split_screen", "ref_persona", "ref_lugar", "ref_doble",
              "nombre_persona", "texto_lateral", "google_fullscreen"}
_SYNC_TIPOS = {"lower_third", "texto_enfasis", "ref_lugar", "ref_persona",
               "google_fullscreen", "broll"}
_YA_APLICO_OVERLAY = {"nombre_persona", "texto_lateral", "ref_doble", "titulo_capitulo",
                      "quote_animado", "lower_third"}
_MAX_LOG = 800
_FONT_BOLD_CANDIDATES = [
    "/Windows/Fonts/Impact.ttf", "/Windows/Fonts/ariblk.ttf",
    "/Windows/Fonts/arialbd.ttf", "/Windows/Fonts/calibrib.ttf",
...
_FONT_REGULAR_CANDIDATES = [
    "/Windows/Fonts/arialbd.ttf", "/Windows/Fonts/arial.ttf",
    "/Windows/Fonts/calibri.ttf", "/Windows/Fonts/verdana.ttf",
...
def _resolve_ref_images(escenas: list[dict], tmp_ref_dir: Path,
                         pexels_key: str, unsplash_key: str) -> dict:
    ...
    esc_with_refs = [
        e for e in escenas
        if e.get("ref_image_b64") or e.get("ref_image_url")
...
def start_render(*, project_name: str, escenas: list[dict], resolucion: str = "1920x1080",
    ...
    audios = (sorted(audio_dir.glob("*"), key=lambda f: f.stat().st_mtime, reverse=True)
    ...
    job_registry.create_job(job_id, {"id": job_id, "estado": "procesando", "progreso": 0,
                                      "mensaje": "Iniciando render enriquecido...", "logs": [],
                                      "video_url": None, "inicio": time.time()})

    threading.Thread(
        target=_procesar_render_enriquecido,
        args=(job_id, project_name, escenas, ref_images, audio_path, resolucion,
              transicion, trans_dur, str(out_dir), str(tmp_ref_dir), pexels_key, unsplash_key),
        daemon=True,
    ).start()
...
def _procesar_render_enriquecido(job_id, proj_name, escenas, ref_images, audio_path,
                                  resolucion, transicion, trans_dur, out_dir, tmp_ref_dir,
    ...
    try:
        ...
        def _on_fallback(unmatched, total, last_ok, dur):
            log(f"[WARNING] Word-level anomalo ({unmatched}/{total} sin match) "
        ...
        if last_real_idx >= 0 and last_real_fin < dur_audio - 1.0:
            ...
            huerfanas = [ei for ei in sorted(ts_map.keys())
            ...
            if huerfanas and tiempo_restante > len(huerfanas) * 0.5:
                ...
                for k, ei in enumerate(huerfanas):
                    ...
                    ts_map[ei] = {"inicio": t_cursor, "fin": round(t_cursor + new_dur, 3), "duracion": new_dur}
                    t_cursor += new_dur
        ...
        def _ct(s, mx=None):
            ...
            o = (o.replace("\\", "").replace("'", "").replace('"', "")
                 .replace("[", "").replace("]", "").replace("%", "pct").strip())
        ...
        def _bsb(wr, hr, col="black"):
            return (f"scale={wr}:{hr}:force_original_aspect_ratio=decrease,"
        ...
        def _apply_timed_overlay(seg_out, vf_str, t_start, t_end, dur_clip, tag, tout=90):
            ...
            run(["ffmpeg", "-y", "-i", seg_out, "-ss", "0", "-t", f"{t_start:.3f}",
                 "-c:v", "libx264", "-preset", "fast", "-crf", "22",
                 "-pix_fmt", "yuv420p", "-r", "24", "-an", sa], f"{tag}-A", timeout=60)
            if os.path.exists(sa):
                parts.append(sa)

            dur_b = max(0.1, t_end - t_start)
            run(["ffmpeg", "-y", "-i", seg_out, "-ss", f"{t_start:.3f}", "-t", f"{dur_b:.3f}",
                 "-c:v", "libx264", "-preset", "fast", "-crf", "22",
                 "-pix_fmt", "yuv420p", "-r", "24", "-an", ebs], f"{tag}-Bsrc", timeout=60)
            ...
            if dur_c > 0.15:
                run(["ffmpeg", "-y", "-i", seg_out, "-ss", f"{t_end:.3f}", "-t", f"{dur_c:.3f}",
                     "-c:v", "libx264", "-preset", "fast", "-crf", "22",
                     "-pix_fmt", "yuv420p", "-r", "24", "-an", sc], f"{tag}-C", timeout=60)
            ...
            if len(parts) >= 2:
                ...
                run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", lf,
                     "-c:v", "libx264", "-preset", "fast", "-crf", "22",
                     "-pix_fmt", "yuv420p", "-r", "24", "-an", tmp], f"{tag}-concat", timeout=tout)
                if os.path.exists(tmp):
                    os.replace(tmp, seg_out)
                    return True
            elif len(parts) == 1:
        ...
        def _find_word_offset(keywords, t_scene_start, t_scene_end, show_dur=None):
            ...
            scene_words = [w for w in all_words
                           if t_scene_start - 0.05 <= float(w.get("start", 0)) <= t_scene_end + 0.05]
            if not scene_words:
                return None
        ...
        _WORD_WINDOW_GENERIC = {"actualidad", "siglo", "epoca", "periodo", "vista", "foto",
                                "imagen", "norte", "sur", "este", "oeste", "circa", "aprox"}
        _WORD_WINDOW_STOP = {"para", "este", "esta", "como", "pero", "cada", "solo", "todo", "que",
                             "con", "sin", "por", "entre", "sobre", "desde", "hasta", "hay", "fue",
                             "era", "son", "sus", "muy", "bien", "nada", "algo", "cuando", "donde",
                             "aunque", "siempre", "nunca", "año", "años", "vez", "veces", "parte",
                             "lugar", "tiempo", "vida", "de", "la", "el", "en", "al", "se", "su",
                             "un", "es", "no", "lo", "una", "les", "han", "sido", "bajo", "toldos",
                             "nocturno", "convoy", "pantalla", "imagen"}
        ...
        def _word_window(overlay_text, scene_texto, t_start_abs, t_end_abs, dur_clip, search_last=False):
            ...
            kws_ov = [p.strip().lower() for p in re.split(r"[,\s%\.]+", overlay_text or "") if len(p.strip()) >= 2]
            kws_ov = [k for k in kws_ov if k not in _WORD_WINDOW_GENERIC]

            kws_txt = [w.lower() for w in re.findall(r"[a-záéíóúüñA-ZÁÉÍÓÚÜÑ]{4,}", scene_texto or "")
            ...
            scene_words = [w for w in all_words
                           if t_start_abs - 0.05 <= float(w.get("start", 0)) <= t_end_abs + 0.05]
            if not scene_words:
                return (0.0, dur_clip)
        ...
        for i, esc in enumerate(escenas):
            ...
            otxt = _ct(otxt_raw, 150) if tipo == "quote_animado" else _ct_short(otxt_raw, max_words=5, max_chars=35)
            ...
            if not img_path:
                ...
                try:
                    run(["ffmpeg", "-y", "-f", "lavfi", "-i", f"color=c=black:size={w_res}x{h_res}:rate=1",
                         "-frames:v", "1", bg_img], f"bg_dyn_{i}", timeout=15)
            ...
            try:
                ...
                if tipo == "intro_dinamica":
                    ...
                    d2 = f"{dur_clip:.4f}"
                    vfi = (f"scale={su2}:{sh2}:force_original_aspect_ratio=increase,"
                           f"crop={w_res}:{h_res}:"
                    ...
                    _enc(seg_out, (f"scale={w_res}:{h_res}:force_original_aspect_ratio=decrease,"
                                   f"pad={w_res}:{h_res}:(ow-iw)/2:(oh-ih)/2:color=black,"
                                   f"setsar=1,fps=24,"
                                   f"fade=t=in:st=0:d={tsd:.3f},setpts=PTS-STARTPTS"),
                         dur_clip, img_path, crf=26)
                    if fp_b:
                        ...
                        dt = (f"colorchannelmixer=rr=0.15:gg=0.15:bb=0.15,"
                    ...
                    slide_vf = (f"scale={hw}:{h_res}:force_original_aspect_ratio=decrease,"
                    ...
                    fc_n = ("[0:v]setpts=PTS-STARTPTS[L];"
                            "[1:v]setpts=PTS-STARTPTS[R];"
                            "[L][R]hstack=inputs=2[v]")
                    _enc(seg_out, None, dur_clip, ["-i", left_vid, "-i", wr_vid],
                         fc=fc_n, mv="[v]", crf=22, tout=60)
                    if fp_b and otxt:
                        nl = _wr(otxt[:35], 16)
                        sn = _sf(nl)
                        fn = int(w_res / 13)
                        xr = hw + int(hw * 0.08)
                        ym = int(h_res * 0.28)
                        dt_nom = (f"drawtext=fontfile='{fp_b}':text='{sn}':"
                        ...
                        if tsec and fp_r:
                            ...
                            dt_nom += (f",drawtext=fontfile='{fp_r}':text='{tsec}':"
                                       f"fontcolor=black@0.52:fontsize={fs2}:x={xr}:y={ys}")
                        ws_nom, we_nom = _word_window(otxt, esc.get("texto", ""), t_scene_start, t_scene_end, dur_clip)
                    ...
                    fc_t = (f"[0:v]setpts=PTS-STARTPTS[bg];"
                    ...
                    _enc(seg_out, None, dur_clip, ["-i", bg_vid, "-loop", "1", "-t", str(dur_clip), "-i", isrc],
                         fc=fc_t, mv="[v]", crf=24, tout=90)
                    if fp_r and otxt:
                        fs_t = int(w_res / 10)
                        ln = _wr(otxt[:30], 20)
                        st = _sf(ln)
                        dt_lat = (f"drawtext=fontfile='{fp_r}':text='{st}':"
                        ...
                        ws_lat, we_lat = _word_window(otxt, esc.get("texto", ""), t_scene_start, t_scene_end, dur_clip)
                    ...
                    _enc(seg_out, None, dur_clip,
                         ["-i", bg_vid, "-loop", "1", "-t", str(dur_clip), "-i", i1,
                          "-loop", "1", "-t", str(dur_clip), "-i", i2],
                         fc=fc_d, mv="[v]", crf=24, tout=120)
                    if fp_b:
                        fs_l = int(w_res / 22)
                        yl = int(h_res * 0.06)
                        dt = ""
                        if sl1:
                            dt += (f"drawtext=fontfile='{fp_b}':text='{sl1}':"
                                   f"fontcolor=red@0.95:fontsize={fs_l}:x={x1}:y={yl}:"
                                   f"shadowcolor=white@0.5:shadowx=1:shadowy=1,")
                        if sl2:
                            dt += (f"drawtext=fontfile='{fp_b}':text='{sl2}':"
                    ...
                    rw_s, rw_e = _word_window(ref_ov_txt, esc.get("texto", ""),
                                               t_scene_start, t_scene_end, dur_clip, search_last=True)
                    ref_show_from = rw_s
                    min_ref_dur = 2.0
                    ...
                    if rw_s <= 0.10:
                        ...
                        ref_fade_vf = (f"scale={w_res}:{h_res}:force_original_aspect_ratio=decrease,"
                                       f"pad={w_res}:{h_res}:(ow-iw)/2:(oh-ih)/2:color=black,"
                                       f"setsar=1,fps=24,"
                        ...
                        if os.path.exists(kb_part) and os.path.exists(ref_part):
                            ...
                            run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", ref_list,
                                 "-c:v", "libx264", "-preset", "fast", "-crf", "22",
                                 "-pix_fmt", "yuv420p", "-r", "24", "-an", seg_out],
                                "ref-concat", timeout=120)
                    ...
                    qbg = (f"scale={w_res}:{h_res}:force_original_aspect_ratio=decrease,"
                           f"pad={w_res}:{h_res}:(ow-iw)/2:(oh-ih)/2:color=black,"
                    ...
                    if fp_r and otxt:
                        ...
                        dt = (f"drawbox=x=0:y={by}:w=iw:h={bh}:color=black@0.70:t=fill,"
                    ...
                    if fp_b and otxt:
                        ...
                        bw = int(w_res * 0.45)
                        pt = int(h_res * 0.012)
                        dt_l3 = (f"drawbox=x={bx}:y={by}:w={bw}:h={bh}:color=black@0.78:t=fill,"
                        ...
                        if tsec and fp_r:
                            ...
                            dt_l3 += (f",drawtext=fontfile='{fp_r}':text='{tsec}':"
                                      f"fontcolor=0x{cacc}@0.85:fontsize={fsu}:"
                                      f"x={bx + int(w_res * 0.012)}:y={ysu}")
                        ws_l3, we_l3 = _word_window(otxt, esc.get("texto", ""), t_scene_start, t_scene_end, dur_clip)
                    ...
                    fc_b = (f"[0:v]setpts=PTS-STARTPTS[bg];"
                    ...
                    _enc(seg_out, None, dur_clip, ["-i", kb_base, "-loop", "1", "-t", str(dur_clip), "-i", rp],
                         fc=fc_b, mv="[v]", crf=24, tout=90)
                ...
                if otxt and fp_b and tipo not in _YA_APLICO_OVERLAY and os.path.exists(seg_out):
                    ...
                    if tipo == "texto_enfasis":
                        ...
                        by = (h_res - bh) // 2
                        dt_enf = (f"drawbox=x=0:y={by}:w=iw:h={bh}:color=black@0.85:t=fill,"
                        ...
                        enf_s, enf_e = _word_window(otxt, esc.get("texto", ""), t_scene_start, t_scene_end, dur_clip)
                        ...
                        lh = int(ft * 1.35)
                        dt_p = [f"drawbox=x=0:y={py2}:w=iw:h={ph2}:color=black@0.72:t=fill",
                        ...
                        fs = int(w_res / 24)
                        pv = int(h_res * 0.020)
                        dt_ref = (f"drawtext=fontfile='{fp_b}':text='{sf}':"
                                  f"fontcolor=white@1.0:fontsize={fs}:"
                        ...
                        ws_ref, we_ref = _word_window(otxt, esc.get("texto", ""), t_scene_start, t_scene_end, dur_clip)
                        ...
                        auto_pos = ["bottom_center", "bottom_left", "bottom_center",
                                    "top_center", "bottom_center", "bottom_left"]
                        xy = ppos.get(opos, ppos.get(auto_pos[i % len(auto_pos)],
                                      f"x=(w-text_w)/2:y=h-text_h-{int(h_res * 0.06)}"))
                        dt = (f"drawtext=fontfile='{fp_r}':text='{sf}':"
                ...
                if rlbl and fp_r and os.path.exists(seg_out):
                    ...
                    rdt = (f"drawtext=fontfile='{fp_r}':text='{rlbl}':"
        ...
        def _do_concat(paths, out_p, timeout=300):
            ...
            run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", lst,
                 ...
                 "-movflags", "+faststart", out_p], "concat_list", timeout=timeout)
        ...
        try:
            ...
            run(["ffmpeg", "-y", "-i", concat_out, "-i", audio_path,
                 "-c:v", "copy", "-c:a", "aac", "-b:a", "192k",
                 "-shortest", "-movflags", "+faststart", video_final],
                "mezcla audio fallback", timeout=180)
        ...
        job_registry.update_job(job_id, estado="completado",
                                 video_url=f"/api/descargar_render/{job_id}",
                                 video_path=video_final, size_mb=round(size_mb, 1))
