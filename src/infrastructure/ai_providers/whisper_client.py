...
def _split_api_segmentos(all_words: list[dict], tr_segments) -> list[dict]:
    ...
    if not all_words:
        return [{"start": float(s.start), "end": float(s.end), "text": s.text, "words": []}
                for s in (tr_segments or [])]
    segmentos = []
    for s in (tr_segments or []):
        seg_words = [w for w in all_words
                     if float(s.start) - 0.05 <= w["start"] <= float(s.end) + 0.05]
        if not seg_words:
            segmentos.append({"start": float(s.start), "end": float(s.end),
                               "text": s.text, "words": []})
            continue
        cur = []
        for k, w in enumerate(seg_words):
            ...
            if (ends_s or has_pause or is_last) and cur:
                ...
                cur = []
    return segmentos or [{"start": float(s.start), "end": float(s.end),
                          "text": s.text, "words": []} for s in (tr_segments or [])]
...
def transcribe_api(audio_path: str, language: str = "es") -> tuple[list[dict], list[dict]]:
    ...
    all_words = [{"word": w.word, "start": float(w.start), "end": float(w.end)}
    ...
    for s in aligned["segments"]:
        for w in (s.get("words") or []):
            if w.get("start") is not None:
                all_words.append({"word": w.get("word", ""), "start": float(w["start"]),
                                   "end": float(w.get("end", w["start"]))})
    segmentos = [
        {
            "start": float(s["start"]), "end": float(s["end"]), "text": s.get("text", ""),
            "words": [
                {"word": w.get("word", ""), "start": float(w["start"]), "end": float(w.get("end", w["start"]))}
...
def transcribe_whisperx_replicate(audio_path: str, language: str | None = "es") -> tuple[list[dict], list[dict]]:
    ...
    for s in segs_raw:
        ...
        for w in (s.get("words") or []):
            if w.get("start") is not None:
                wd = {"word": w.get("word", ""), "start": float(w["start"]),
                      "end": float(w.get("end", w["start"]))}
                all_words.append(wd)
                words_s.append(wd)
        segmentos.append({"start": float(s.get("start", 0)), "end": float(s.get("end", 0)),
                           "text": s.get("text", ""), "words": words_s})
    return segmentos, all_words
